#ifndef __AVUOS_MEMORYBITMAP_HPP_
#define __AVUOS_MEMORYBITMAP_HPP_
...
public:
	MemoryBitmap()
	{
		// Doing this because the compiler doesn't like the array initializer :^)
		// FIXME: Take a look at this again, may be an issue with the toolchain
		for (auto & b : bitmap) {
			b = 0;
		}
	}
	...
	size_t find_pages(size_t num_pages, size_t start_index)
	{
		...
		while (true) {
			// check if the chunk of pages starting at cur_page
			bool all_free = true;
...
#endif // __AVUOS_MEMORYBITMAP_HPP_