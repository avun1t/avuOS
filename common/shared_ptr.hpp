...
#ifndef __AVUOS_SHARED_PTR_HPP_
#define __AVUOS_SHARED_PTR_HPP_
 ...
 */
	...
	public:
		...
		shared_ptr(void) throw() : px(NULL), pn() // never throws
		{}

		/// @brief Constructor with the provided pointer to manage
		explicit shared_ptr(T* p) : pn() // may throw std::bad_alloc
		...
		template <class U>
		shared_ptr(const shared_ptr<U>& ptr) throw() : pn(ptr.pn) // never throws (see comment below)
		//px(ptr.px),
		{
		...
		shared_ptr(const shared_ptr& ptr) throw() : pn(ptr.pn) // never throws (see comment below)
		//px(ptr.px),
		{
