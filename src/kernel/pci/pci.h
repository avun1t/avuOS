...
namespace PCI {
	...
	public:
		uint8_t bus;
		uint8_t slot;
		uint8_t flags;  // Bit 0: exists
						// Bit 1: Multiple devices with this device's class, subclass and progIF
	};

	uint16_t read_word(uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset);
	uint16_t get_vendor(uint8_t bus, uint8_t slot, uint8_t function);
	Device get_device(uint8_t deviceClass, uint8_t subClass, uint8_t progIF);
	void pci_debug();
	void print_pci_class_code(uint8_t classCode, uint8_t subClass, uint8_t progIF);
};
