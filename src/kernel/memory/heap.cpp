...
uint8_t heap_space[0x100000]; // 1 MiB of heap space
uint8_t frames[0x2000]; // 16 byte frames, 1 bit each. 1 = used, 0 = free
Frame frbuf;
FrameSet frsbuf;
...
FrameSet first_available_frameset(uint32_t len)
{
	...
	PANIC("NO_VAR_SPACE", "The heap doesn't have enough consecutive frames to fit in a variable.", false);
...
FrameSet fsalloc(uint32_t len)
{
	...
	for (int i = 0; i < frsbuf.len; i++) {
		buf = get_frame(i+frsbuf.start.num);
		frames[buf.set] |= 1 << buf.pos;
	}

	FrameSet fs;
	fs.start = get_frame(frsbuf.start.num);
	fs.len = len;
	return fs;
}
